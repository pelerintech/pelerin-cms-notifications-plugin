...
const PAGE_PATH = join(__dirname, '../../src/pages/admin/templates/index.astro');
...
/**
 * Regression guard: the client `<script>` in the templates/index.astro page must be
 ...
 */
describe('admin templates/index.astro page — client <script> syntax', () => {
  it('transforms with esbuild without a syntax/duplicate-declaration error', () => {
    ...
    assert.ok(scriptMatches.length > 0, 'expected at least one <script> block in templates/index.astro');
    ...
    assert.equal(
      exitCode,
      0,
      `client <script> in templates/index.astro has a syntax error — the entire client script is disabled at parse time.\nesbuild output:\n${combined}`,
    );
  });
});
