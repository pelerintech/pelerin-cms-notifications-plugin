...
const PAGE_PATH = join(__dirname, '../../src/pages/admin/providers/[name].astro');
...
/**
 * Regression guard: the client `<script>` in the providers/[name].astro page must be
 ...
 */
describe('admin providers/[name].astro page — client <script> syntax', () => {
  it('transforms with esbuild without a syntax/duplicate-declaration error', () => {
    ...
    assert.ok(scriptMatches.length > 0, 'expected at least one <script> block in providers/[name].astro');
    ...
    assert.equal(
      exitCode,
      0,
      `client <script> in providers/[name].astro has a syntax error — the entire client script is disabled at parse time.\nesbuild output:\n${combined}`,
    );
  });
});
