...
/**
 * Regression guard: the client `<script>` in the logs/index.astro page must be
 ...
 */
describe('admin logs/index.astro page — client <script> syntax', () => {
  it('transforms with esbuild without a syntax/duplicate-declaration error', () => {
    ...
    assert.ok(scriptMatches.length > 0, 'expected at least one <script> block in logs/index.astro');
