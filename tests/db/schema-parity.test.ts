 ...
 *
 * Recognizes `const <name> = defineTable({ ... })` blocks and
...
function parseAstroDbConfig(source: string): Record<string, Record<string, { type: string; optional: boolean }>> {
  ...
  // Stop at the first `\n  },` — the columns block close. This works whether or
  // not uniqueConstraints (or other props) follow columns in the defineTable.
